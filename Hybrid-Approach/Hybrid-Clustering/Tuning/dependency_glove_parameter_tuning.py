"""dependency_glove_parameter_tuning.py

HDBSCAN parameter tuning for SecureBERT Hybrid Approach embeddings.
The embeddings combine SecureBERT and dependency-based features, and are averaged before clustering.
...
# Load data
FILENAME = "dependency_glove.csv"
# CHANGE FILE PATH HERE
encodings = pd.read_csv("/scratch/alpine/anle7858/Hybrid_Approach/" + FILENAME)

# Stack a list of 1D arrays into a 2D matrix for clustering
X = np.vstack(encodings["article_glove_mean"].apply(
    lambda x: np.fromstring(x.strip("[]"), sep=" ") # convert embedding from str to numpy
))
print(f"The shape of X (num_chunks, embedding_dim) is: {(X.shape)}") # (num_chunks=13074, embedding_dim=768)
...
SAVE_FILENAME = "dependency_glove_tuning_results.csv"
# CHANGE FILE PATH HERE
...
# run HDBSCAN with best parameters (dependency_glove_clustering.py)
